...
describe("action routes", () => {
  ...
  describe("GET /digipet/ignore", () => {
    test("if the user has a digipet, it responds with a message about ignoring it", async () => {
      // setup: reset digipet
      setDigipet(INITIAL_DIGIPET);
...
describe("GET /digipet/rehome", () => {
  test("if the user has no digipet, it responds with a message explaining that a digipet can't be rehomeed whilst the user has none", async () => {
    // setup
  ...
  test("if the user has a digipet, it responds with a message about successfully rehomeing a digipet and calls rehomeDigipet", async () => {
    // setup
    if (jest.isMockFunction(rehomeDigipet) /* type guard */) {
      rehomeDigipet.mockReset();
    }
